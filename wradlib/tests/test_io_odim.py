...
def get_measured_volume(file, get_loader, format):
    ...
    yield io.xarray.open_odim(h5file, loader=get_loader,
                              flavour=format)
...
def get_synthetic_volume(name, get_loader, **kwargs):
    ...
    with h5py.File(str(tmp_local), 'w') as f:
        data = globals()[name]()
        write_group(f, data)
    yield io.xarray.open_odim(tmp_local, loader=get_loader,
...
def create_sweep_repr(format, azi, range, ele, mom):
    ...
    repr = ''.join([f'<wradlib.XRadSweep{format}>\n',
                    f'Dimensions: (azimuth: {azi}, range: {range})\n',
                    f'Elevation: ({ele})\n',
                    f'Moment(s): ({", ".join(mom)})'])
    return repr
            ...
            for i, ts in enumerate(vol):
                for j, swp in enumerate(ts):
                    for k, mom in enumerate(swp):
                        repr = create_moment_repr(self.azimuths[i],
                                                  self.ranges[i],
                                                  self.elevations[i],
                                                  self.moments[k])
                    ...
                    ncpath = '/'.join([self.dsdesc,
                                       self.mdesc]).format(i + num,
                                                           k + num)
...
class DataSweep(DataMoment):
    ...
    def test_sweeps(self, get_loader):
        ...
        with self.get_volume_data(get_loader) as vol:
            ...
            for i, ts in enumerate(vol):
                for j, swp in enumerate(ts):
                    repr = create_sweep_repr(self.format, self.azimuths[i],
                                             self.ranges[i],
                                             self.elevations[i],
                                             self.moments)
                    ...
                    # mixins
                    attrs = get_group_attrs(self.data,
                                            self.dsdesc.format(i + num),
                                            'how')
...
class DataTimeSeries(DataSweep):
    ...
    def test_timeseries(self, get_loader):
        ...
        with self.get_volume_data(get_loader) as vol:
            ...
            num = 1 if self.format == 'ODIM' else 0
            for i, ts in enumerate(vol):
                repr = create_timeseries_repr(self.volumes,
                                              self.azimuths[i],
                                              self.ranges[i],
                                              self.elevations[i])
    ...
    def test_volumes(self, get_loader):
        ...
        with self.get_volume_data(get_loader) as vol:
            ...
            xr.testing.assert_equal(vol.site,
                                    create_site(self.data['where']['attrs']))
...
class TestKNMIVolume(MeasuredDataVolume):
    ...
    sweeps = 14
    moments = ['DBZH']
    elevations = [0.3, 0.4, 0.8, 1.1, 2.0, 3.0, 4.5, 6.0, 8.0, 10.0, 12.0,
                  15.0, 20.0, 25.0]
    azimuths = [360] * sweeps
    ranges = [320, 240, 240, 240, 240, 340, 340, 300, 300, 240, 240, 240, 240,
              240]
...
def base_odim_data_double_th():
    ...
    for i, grp in enumerate(dataset):
        ...
        sub['how']['attrs']['startazA'][10] = sub['how']['attrs']['startazA'][9]
        sub['how']['attrs']['stopazA'][10] = sub['how']['attrs']['stopazA'][9]
    return data
...
class TestNotOdimCompliant():
    ...
    def test_double_rays(self):
        ...
        pvol = wrl.io.xarray.open_odim([dbzh,th],parallel=False)
        sweep = pvol[0].data
